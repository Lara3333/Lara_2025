...
class NPC2 extends Player {
    ...
    checkProximityToNPC() {
        ...
        if (players.length > 0 && npc2) {
            players.forEach(player => {
                // The Euclidean distance between two points in a 2D space
                var distance = Math.sqrt(
                    Math.pow(player.position.x - npc2.position.x, 2) + Math.pow(player.position.y - npc2.position.y, 2)
                );
                // The distance is less than 100 pixels
                if (player != npc2 && distance <= 100) {
                    names.push(player.spriteData.name);
                }
            });
            // Join all player names inside the proximity
            if (names.length > 0) {
                this.handleResponse(`I hate u, ${names.join(', ')}`);
            }
        }
    }
}
